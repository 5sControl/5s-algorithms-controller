...
app.use('/run', async (req, res) => {
    ...
    // python algorithms
    try {
        if (req.body.algorithm === 'min_max_control' && MIN_MAX_PYTHON || req.body.algorithm === 'idle_control' && IDLE_PYTHON) {
    ...
    try {
        ...
        const createdAlgorithm = new currentAlgorithm(cameras[reqBody.camera_url].camera, algorithm, extra)
    ...
    try {
        ...
        if (pid && algorithms[pid] && !algorithms[pid]?.container) {
            ...
            if (cameras[camera_url].algorithms.includes(algorithm)) {
                cameras[camera_url].algorithms = cameras[camera_url].algorithms.filter(algoName => algoName !== algorithm);
            }
            res.json({'status': true})
            delete algorithms[pid]
            return
